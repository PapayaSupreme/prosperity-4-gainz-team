...
class TomatoesAdaptiveMarketMaker(StatefulStrategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        for ask_price, ask_volume_raw in sell_orders:
            if buy_left <= 0 or ask_price > reservation - take_threshold:
                break
            size = min(buy_left, -ask_volume_raw, 12)
            self.buy(ask_price, size)
            buy_left -= size

        for bid_price, bid_volume in buy_orders:
            if sell_left <= 0 or bid_price < reservation + take_threshold:
                break
            size = min(sell_left, bid_volume, 12)
            self.sell(bid_price, size)
            sell_left -= size
